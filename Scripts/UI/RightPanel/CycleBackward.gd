...
func _input(event):
	...
	if cycle_id >= 0 and cycle_id < Global.settings_dict.cycles.size():
		Global.settings_dict.cycles[cycle_id].backward = event.duplicate()
	update_key_text()
	button_pressed = false
...
func update_key_text():
	if %CycleChoice.get_selected_id() > 0:
		if Global.settings_dict.cycles[%CycleChoice.get_selected_id()-1].backward != null:
			self.text = InputDisplayName.text(Global.settings_dict.cycles[%CycleChoice.get_selected_id()-1].backward)
		else:
