...
func set_mc_wobble():
	position.x = lerp(position.x, sin(tick*state_param_mc.xFrq)*state_param_mo.xAmp, 0.08)
	position.y = lerp(position.y, sin(tick*state_param_mc.yFrq)*state_param_mo.yAmp, 0.08)
